...
import { Database, Code, Download, RefreshCw, AlertTriangle, Settings, Upload, Info, Layout, BarChart3 } from 'lucide-react';
...
export default function TimeSeriesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('unknown') || errorMessage.includes('not found') || errorMessage.includes('does not exist')) {
        alert(
          `❌ Table Not Found\n\n` +
          `The table "${tableConfig.schemaName}.${tableConfig.tableName}" does not exist.\n\n` +
          `What you can do:\n` +
          `1. Click "Configure" to select a different table\n` +
          `2. Click "Configure" → "Create New" to create this table\n` +
          `3. Click "Import" to create and populate the table from CSV/JSON\n\n` +
  ...
  const handleSaveDashboard = (template: any) => {
    ...
    alert(`✅ Dashboard Saved!\n\n"${template.name}" has been saved successfully.\n\nClick "View Dashboards" to see your saved dashboards.`);
  };
      ...
      <div className="border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white px-6 py-4 dark:border-gray-700 dark:from-gray-800 dark:to-gray-800">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => setShowCRUDInfo(true)}
              className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 animate-pulse hover:animate-none group relative"
            >
              <Info className="h-4 w-4" />
              SQL Guide
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 px-3 py-2 text-xs bg-gray-900 dark:bg-gray-700 text-white rounded-lg shadow-lg z-50">
                30+ copy-paste SQL examples for CREATE, READ, UPDATE, DELETE operations
              </span>
            </button>
            <button
              onClick={() => setShowDashboardBuilder(true)}
              className="flex items-center gap-2 rounded-lg bg-purple-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-600 group relative"
            >
              <Layout className="h-4 w-4" />
              Create Dashboard
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 px-3 py-2 text-xs bg-gray-900 dark:bg-gray-700 text-white rounded-lg shadow-lg z-50">
                Build custom dashboards with 7 chart types and save as templates
              </span>
            </button>
            {savedTemplates.length > 0 && (
              <button
                onClick={() => setShowDashboardViewer(true)}
                className="flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 group relative"
              >
                <BarChart3 className="h-4 w-4" />
                View Dashboards ({savedTemplates.length})
                <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 px-3 py-2 text-xs bg-gray-900 dark:bg-gray-700 text-white rounded-lg shadow-lg z-50">
                  Execute your saved dashboards with real-time data
                </span>
              </button>
            )}
            <button
              onClick={() => setShowTableConfig(true)}
              className="flex items-center gap-2 rounded-lg bg-gray-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-gray-700 dark:bg-gray-500 dark:hover:bg-gray-600 group relative"
            >
              <Settings className="h-4 w-4" />
              Configure
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 px-3 py-2 text-xs bg-gray-900 dark:bg-gray-700 text-white rounded-lg shadow-lg z-50">
                Change data source, select different table, or create new table
              </span>
            </button>
            <button
            ...
            >
              ...
              Import
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 px-3 py-2 text-xs bg-gray-900 dark:bg-gray-700 text-white rounded-lg shadow-lg z-50">
                Upload CSV/JSON files - auto-creates tables and imports data
              </span>
            </button>
            <button
              onClick={loadData}
              disabled={loading}
              className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 disabled:opacity-50 dark:bg-blue-500 dark:hover:bg-blue-600 group relative"
            >
              <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />
              Refresh
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 px-3 py-2 text-xs bg-gray-900 dark:bg-gray-700 text-white rounded-lg shadow-lg z-50">
                Reload data from database with current configuration
              </span>
            </button>
            <button
              onClick={handleExportData}
              disabled={series.length === 0}
              className="flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-green-500 dark:hover:bg-green-600 group relative"
            >
              <Download className="h-4 w-4" />
              Export
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 px-3 py-2 text-xs bg-gray-900 dark:bg-gray-700 text-white rounded-lg shadow-lg z-50">
                Export data in CSV, JSON, Excel, or SQL format
              </span>
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
          ...
          {loading ? (
            ...
            <div className="flex h-80 items-center justify-center bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-900 dark:to-blue-900/10 rounded-lg">
              <div className="text-center px-6">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 max-w-md">
                  Configure your metrics and time range below, then click the <strong>Refresh</strong> button to load and visualize your time-series data.
                </p>
                <div className="flex items-center justify-center gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-2.5 bg-gray-600 hover:bg-gray-700 text-white font-medium rounded-lg transition-colors"
                  >
            ...
            <div className="flex h-80 items-center justify-center bg-gray-50 dark:bg-gray-900 rounded-lg">
              <div className="text-center px-6">
                <AlertTriangle className="h-16 w-16 text-yellow-500 dark:text-yellow-600 mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                  No Data Found
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 max-w-md">
                  No time-series data was found for the selected date range. Try adjusting your time range or check if your table has data.
                </p>
                <div className="flex items-center justify-center gap-3">
                  <button
                    onClick={() => setShowImportDialog(true)}
