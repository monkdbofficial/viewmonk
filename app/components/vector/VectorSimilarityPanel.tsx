...
export default function VectorSimilarityPanel() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      try {
        if (referenceVector.trim().startsWith('[')) {
          vectorArray = JSON.parse(referenceVector);
        } else {
          vectorArray = referenceVector.split(',').map(v => parseFloat(v.trim()));
        }
      } catch (e) {
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Enter reference vector as JSON array: [0.1, 0.2, 0.3, ...] or comma-separated: 0.1, 0.2, 0.3, ..."
              rows={3}
